...
import { ShoppingCart, Shield, Hammer, MapPin, Phone, Clock, ChevronRight, Search, MessageCircle, CheckCircle } from 'lucide-react';
...
function MainApp() {
  ...
  // ✅ Lazy init — avoids recreating Audio object unnecessarily
  const audioRef = useRef(null);
  const getAudio = () => {
    if (!audioRef.current) {
      audioRef.current = new Audio('https://www.soundjay.com/transportation/car-revving-1.mp3');
    }
    return audioRef.current;
  };

  const handleEnterSite = () => {
    getAudio().play().catch(() => {}); // ✅ Catch autoplay block errors
    setHasEntered(true);
  };
  ...
  return (
    <div className="min-h-screen bg-[#F8F9FA] font-sans text-[#0A192F]">
      ...
      <AnimatePresence>
        {toast && (
          <motion.div
            ...
            exit={{ opacity: 0, y: -20 }}
            className="fixed top-24 left-1/2 -translate-x-1/2 z-[200] bg-[#0A192F] text-white px-6 py-3 rounded-full shadow-2xl flex items-center gap-3 text-sm font-bold whitespace-nowrap"
          >
      ...
      <a
        href="https://wa.me/260972626617"
        target="_blank"
        rel="noopener noreferrer"
        className="fixed bottom-6 right-6 z-40 bg-[#25D366] text-white w-14 h-14 rounded-full flex items-center justify-center shadow-2xl hover:scale-110 active:scale-95 transition-transform"
      ...
      <nav className="bg-white/80 backdrop-blur-md shadow-sm sticky top-0 z-30 border-b border-gray-100">
        <div className="max-w-6xl mx-auto px-4 py-4 flex justify-between items-center">
          ...
          <div className="flex items-center gap-4 md:gap-6">
            ...
            <div className="flex gap-4 md:gap-6 font-bold text-sm uppercase tracking-widest">
      ...
      <AnimatePresence mode="wait">
        {currentPage === 'home' ? (
          <motion.div key="home" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
            ...
            <header className="relative h-[80vh] flex items-center justify-center text-white overflow-hidden">
              <div
                ...
                style={{ backgroundImage: `url('https://images.unsplash.com/photo-1603553329474-99f95f35394f?auto=format&fit=crop&q=80&w=2000')` }}
              />
              ...
              <div className="relative z-10 max-w-4xl mx-auto px-4 text-center">
                <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }}>
                  ...
                  <button
                    ...
                    className="w-full md:w-auto bg-blue-600 hover:bg-blue-700 text-white font-black px-12 py-5 rounded-2xl transition-all shadow-[0_20px_50px_rgba(37,99,235,0.3)] uppercase tracking-widest flex items-center justify-center gap-3 group mx-auto"
                  >
          ...
          >
            {/* STORE HEADER */}
            <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-10 border-b-2 border-gray-100 pb-10">
              <div>
            ...
            <div className="flex flex-col md:flex-row gap-4 mb-8">
              <div className="relative flex-1">
                ...
                <input
                  ...
                  className="w-full pl-10 pr-4 py-3 border border-gray-200 rounded-xl bg-white focus:outline-none focus:ring-2 focus:ring-blue-600 text-sm font-medium"
                />
              </div>
              <select
                value={sortOrder}
                onChange={e => setSortOrder(e.target.value)}
                className="px-4 py-3 border border-gray-200 rounded-xl bg-white focus:outline-none focus:ring-2 focus:ring-blue-600 text-sm font-bold text-gray-600 cursor-pointer"
              >
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
                {filteredProducts.map((product, index) => (
                  <motion.div
                    ...
                    className="bg-white rounded-3xl shadow-sm hover:shadow-2xl transition-all duration-500 overflow-hidden border border-gray-100 group"
                  >
                    <div className="overflow-hidden h-56 relative">
                      <img src={product.image} alt={product.name} className="w-full h-full object-cover group-hover:scale-110 transition-transform duration-1000" />
                      <div className="absolute top-4 left-4 bg-[#0A192F] text-white px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-widest">
                        {product.category}
                      </div>
                    </div>
                    <div className="p-6">
                      <h4 className="font-bold text-gray-900 mb-4 h-12 line-clamp-2 text-lg leading-tight group-hover:text-blue-600 transition-colors">
                        {product.name}
                      </h4>
                      <div className="flex justify-between items-center pt-4 border-t border-gray-50">
                        ...
                        <button
                          ...
                          className="bg-blue-600 text-white w-12 h-12 rounded-2xl flex items-center justify-center font-bold text-xl hover:bg-[#0A192F] transition-all shadow-lg active:scale-90"
                        >
                          +
                        </button>
                      </div>
      ...
      <footer className="bg-white border-t border-gray-100 pt-20 pb-10">
        <div className="max-w-6xl mx-auto px-4 text-center">
          ...
          <div className="flex justify-center gap-8 mb-10 text-gray-400 font-bold uppercase text-[10px] tracking-widest">
            {/* ✅ Phone number matches WhatsApp number in CartContext */}
            <div className="flex items-center gap-2"><Phone size={14} className="text-blue-600" /> +260 972 626 617</div>
